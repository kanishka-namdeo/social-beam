...
import { HashtagSchema } from '@/lib/tools/schemas';
import { buildHashtagPrompt } from '@/lib/tools/hashtag-generator';
...
export async function POST(req: Request) {
  ...
  try {
    ...
    log.info('api.request.start', { method: 'POST', path: '/api/tools/hashtag', platform: parsed.data.platform });
    ...
    const prompt = buildHashtagPrompt(parsed.data.topic, parsed.data.platform);
    ...
    try {
      ...
      log.error('api.hashtag.parse_failed', { content: content.slice(0, 200) });
    ...
    log.info('api.request.success', { requestId, categoryCount: result?.categories?.length });
    return nextResponse;
  } catch (error) {
