...
export async function GET(
  ...
  try {
    ...
    const result = await prisma.scraperProcess.findFirst({
    ...
    logger.error("api.processes.get.error", { error: String(error) });
...
export async function DELETE(
  ...
  try {
    ...
    logger.error("api.processes.delete.error", { error: String(error) });
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
